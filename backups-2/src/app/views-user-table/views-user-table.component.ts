...
import { OutputUserAddAndUpdateFormsComponent } from "./user-add-and-update-forms/output-user-add-and-update-forms.component";
...
export class ViewsUserTableComponent implements OnInit {
  ...
  outputsEmitterEvent(outputs: OutputUserAddAndUpdateFormsComponent): void {
    if (outputs.currentListChange) { this.currentList = outputs.currentListChange; }
    if (outputs.userListChange) { this.userList = outputs.userListChange; }
    if (outputs.userAgeChange) { this.userAge = outputs.userAgeChange; }
    if (outputs.userPointsChange) { this.userPoints = outputs.userPointsChange; }
    if (outputs.currentUserIdChange) { this.currentUserId = outputs.currentUserIdChange; }

    if (this.currentAge) { this.onChangeAge(`${this.currentAge}`); }
    if (this.currentPoint) { this.onChangePoint(`${this.currentPoint}`); }
    if (this.activeSort) { this.sorting(this.activeSort); }
    this.redPointOn();
  }
}
