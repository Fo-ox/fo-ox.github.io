...
export class Draw<TYPE> {

    // Отрисовка корня
    drawRoot (root: NodeClass<TYPE>): void {
    ...
    // Отрисовка вершин
    drawNodes (node: NodeClass<TYPE>): void {
        if ( node.Path !== "null") {
    ...
    GenerateRootId (root: NodeClass<TYPE>): void {
        root.Path = "root";
    ...
    GenerateNodeId (node: NodeClass<TYPE>): void {
        if (node.LeftChild) {
