...
export class ControlService {
  ...
  filterStudent(list: TestData, points: number, age: number): User[] {
    const returnArray = [];
    let j = 0;
    if (!points && !age) {
      while (list[j]) {
  ...
  uniquePoinst(list: TestData): number[] {
    let i = 0;
    const returnArray = [];
    while (list[i]) {
  ...
  uniqueAge(list: TestData): number[] {
    let i = 0;
    const returnArray = [];
    while (list[i]) {
      const user: User = list[i];
  ...
  detele(list: User[], id: number): User[] {
    const returnArray = [];
    let j = 0;
    while (list[j]) {
      const user: User = list[j];
  ...
  sortList(list: User[], sortType: string): User[] {
    const returnArray = [];
    let j = 0;
    while (list[j]) {
      const user: User = list[j];
  ...
  addFormUser(list: User[], newUser: FormGroup, id?: number): User[] {
    const returnArray = [];
    if (id) {
      let i = 0;
      while (list[i]) {
