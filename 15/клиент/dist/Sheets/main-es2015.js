...
const user1 = new _model_server_user__WEBPACK_IMPORTED_MODULE_1__["ServerUser"](1, new Date("1994-01-01"), "Никита", "Давидов", "Олегович", 5, "A-100", "8(800)000-10-11", "A@A.RU", "Street-10-A");
const user2 = new _model_server_user__WEBPACK_IMPORTED_MODULE_1__["ServerUser"](2, new Date("1994-04-30"), "Ольга", "Майорова", "Олеговна", 2.38, "A-101");
const user3 = new _model_server_user__WEBPACK_IMPORTED_MODULE_1__["ServerUser"](3, new Date("1992-09-12"), "Андрей", "Дунаев", "Васильевич", 2, "A-105");
const user4 = new _model_server_user__WEBPACK_IMPORTED_MODULE_1__["ServerUser"](4, new Date("1998-12-17"), "Виктор", "Долголетов", "Анатольевич", 1, "A-101");
const user5 = new _model_server_user__WEBPACK_IMPORTED_MODULE_1__["ServerUser"](5, new Date("1995-01-11"), "Анна", "Тихомирова", "Сергеевна", 3.5, "A-101");
const user6 = new _model_server_user__WEBPACK_IMPORTED_MODULE_1__["ServerUser"](6, new Date("1992-05-19"), "Никита", "Гридин", "Юрьевич", 5, "A-103");
const user7 = new _model_server_user__WEBPACK_IMPORTED_MODULE_1__["ServerUser"](7, new Date("1993-08-24"), "Никита", "Шилов", "Максимович", 4.44, "A-102");
const user8 = new _model_server_user__WEBPACK_IMPORTED_MODULE_1__["ServerUser"](8, new Date("1992-12-12"), "Кристина", "Кондракова", "Васильевна", 5, "A-103");
class DebugData {
}
...
let CustomDirective = class CustomDirective {
    constructor(elements, render, service) {
...
class ServerUser {
    constructor(Id, Dates, Name, Surname, Patronymic, Points, Group, Phone, Email, Adress) {
        this._id = Id;
        this._date = Dates;
        this._name = Name;
...
let DateAgePipe = class DateAgePipe {
    transform(value) {
        let word = "";
...
let Factory = class Factory {
    ...
    get currentList() { return this._currentList; }
    set currentList(value) { this._currentList = value; }
    get userList() { return this._userList; }
    set userList(value) { this._userList = value; }
    get userPoints() { return this._userPoints; }
    set userPoints(value) { this._userPoints = value; }
    get userAge() { return this._userAge; }
    set userAge(value) { this._userAge = value; }
    get currentUserId() { return this._currentUserId; }
    set currentUserId(value) { this._currentUserId = value; }
    get currentAge() { return this._currentAge; }
    set currentAge(value) { this._currentAge = value; }
    get currentPoint() { return this._currentPoint; }
    set currentPoint(value) { this._currentPoint = value; }
    get activeSort() { return this._activeSort; }
    set activeSort(value) { this._activeSort = value; }
    transpilatedModel(resolve, status) {
        const returnArray = [];
        let i = 0;
        while (resolve[i]) {
    ...
    transpilatedUser(resolve, status) {
        let returnUser;
        returnUser =
...
let DebugService = class DebugService extends _abstract_service__WEBPACK_IMPORTED_MODULE_5__["Factory"] {
    ...
    // Guard method
    userMayBeChange(userId, router) {
        return new Promise(resolve => {
            let returnUser = null;
            _data_debug_data__WEBPACK_IMPORTED_MODULE_3__["DebugData"].studentList.filter((user) => {
                if (user.id === +userId) {
                    returnUser = user;
                }
            });
    ...
    server_deleteUserId(userId) {
        console.log(userId);
        let returnUser = [];
        returnUser = _data_debug_data__WEBPACK_IMPORTED_MODULE_3__["DebugData"].studentList.filter((user) => {
            if (user.id === +userId) {
                return user;
            }
        });
        _data_debug_data__WEBPACK_IMPORTED_MODULE_3__["DebugData"].studentList = _data_debug_data__WEBPACK_IMPORTED_MODULE_3__["DebugData"].studentList.filter(user => user.id !== userId);
        console.log(_data_debug_data__WEBPACK_IMPORTED_MODULE_3__["DebugData"].studentList);
        return returnUser;
    }
    server_addAndUpdateUser(newUser, userId) {
        let returnUser = [];
        if (userId && newUser) {
            returnUser = _data_debug_data__WEBPACK_IMPORTED_MODULE_3__["DebugData"].studentList.filter((user) => {
                if (user.id === userId) {
            ...
            const listSize = _data_debug_data__WEBPACK_IMPORTED_MODULE_3__["DebugData"].studentList.length;
            let maxindex = 0;
...
let ServerService = class ServerService extends _abstract_service__WEBPACK_IMPORTED_MODULE_3__["Factory"] {
...
const serviceFactory = (http, route) => {
...
let UserAddAndUpdateFormsComponent = class UserAddAndUpdateFormsComponent {
    constructor(controlService, urlService, cdr, router, route) {
    ...
    ngOnInit() {
        ...
        this.ViewDialogFormUpdateSub = this.urlService.ViewDialogFormUpdateSubject.subscribe((res) => {
    ...
    FormSubmit() {
        this.urlService.addFormUser(this.formsAddUser, this.urlService.currentUserId)
            ...
            this.urlService.getAllUserList()
                ...
                if (this.urlService.activeSort && this.urlService.currentList) {
                    this.urlService.sorting(this.urlService.activeSort);
                }
                if (this.urlService.currentList) {
                    this.urlService.redPointOn();
                }
                this.urlService.currentUserId = null;
            });
        });
    ...
    ViewDialogFormUpdate(value) {
        ...
        this.urlService.getUserId(this.urlService.currentUserId)
            .subscribe((res) => {
            const currentUser = this.urlService.transpilatedUser(res, false);
            this.urlService.currentUserId = value;
...
UserAddAndUpdateFormsComponent.ctorParameters = () => [
...
let UserAddAndUpdateFormsModule = class UserAddAndUpdateFormsModule {
};
...
let ViewsUserTableComponent = class ViewsUserTableComponent {
    constructor(controlService, urlService, 
    ...
    get currentClass_tr() { return this._currentClass_tr; }
    set currentClass_tr(cl) { this._currentClass_tr = cl; }
    detect() {
        // this.cdr.detectChanges();
        setTimeout(() => {
            this.cdr.detectChanges();
    ...
    ViewDialogFormUpdate(listId, currentList) {
...
ViewsUserTableComponent.ctorParameters = () => [
...
let ViewsUserTableModule = class ViewsUserTableModule {
};
